...
const ONE_PIXEL_PNG_BASE64 = 'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO9Wl9kAAAAASUVORK5CYII=';
...
function flattenBoxText(box: any): string {
  ...
  return box.lines
    .map((line: any[]) => line.map((seg: any) => String(seg?.text || '')).join(''))
    .join('\n');
}
...
function testSemanticMapping(): void {
  const inputPath = path.resolve('src/formats/markdown/flavors/sample_default.md');
...
function testFormatCompileAndIrEmission(): void {
  ...
  const result = compileMarkdownToVmprint(markdown, inputPath, 'markdown');
  const ir = result.ir;

  assert.equal(ir.layout.fontFamily.length > 0, true);
  assert.equal(result.format, 'markdown');
  assert.ok(ir.styles.d2f_paragraph);
  assert.ok(ir.styles.d2f_heading_1);
...
function testBlockquotePreservesInlineStyling(): void {
  ...
  const ir = compileMarkdownToVmprint(markdown, inputPath, 'markdown').ir;

  const quotes = ir.elements.filter((element) => element.type === 'd2f_blockquote');
  ...
  assert.equal(ir.styles.d2f_inline_code?.fontFamily, 'Cousine');
...
function testReferenceStyleLinksCompileToCitationsAndReferences(): void {
  ...
  assert.ok(serialized.includes('"d2f_references_heading"'), 'expected references heading');
...
function testMarkdownFlavorSelection(): void {
  const markdown = '# Title\n\nParagraph with a [link](https://example.com).';
  const inputPath = 'flavor.md';

  const defaultResult = compileMarkdownToVmprint(markdown, inputPath, 'markdown');
  const academicResult = compileMarkdownToVmprint(markdown, inputPath, 'markdown', 'academic');
...
function testHeadingInlineEmphasisKeepsHeadingScale(): void {
  ...
  const ir = compileMarkdownToVmprint(markdown, inputPath, 'markdown', 'literature').ir;
  const heading = ir.elements.find((element) => element.type === 'd2f_heading_1');
  ...
  assert.equal(serialized.includes('"fontSize":11.4'), false, 'inline emphasis should not reset to body font size');
}
...
function testUnorderedMarkersAreDepthBased(): void {
  ...
  const unordered = ir.elements.filter((element) => String(element.type).startsWith('d2f_list_item_unordered_'));
  assert.ok(unordered.length >= 5, 'expected emitted unordered list rows');

  const topLevelDepth = Math.min(...unordered.map((element) => {
    const parts = String(element.type).split('_');
    return Number(parts[parts.length - 1]);
  }).filter((value) => Number.isFinite(value)));
  const topLevel = unordered.filter((element) => String(element.type).endsWith(`_${topLevelDepth}`));
...
function testFlavorContinuationIndentPolicy(): void {
  ...
  const defaultIr = compileMarkdownToVmprint(markdown, inputPath, 'markdown', 'default').ir;
  const literatureIr = compileMarkdownToVmprint(markdown, inputPath, 'markdown', 'literature').ir;

  const defaultContinuation = defaultIr.styles['d2f_list_item_continuation_1'] as any;
  const literatureContinuation = literatureIr.styles['d2f_list_item_continuation_1'] as any;

  assert.ok(typeof defaultContinuation?.textIndent === 'number', 'expected default continuation indentation');
  assert.ok(typeof literatureContinuation?.textIndent === 'number', 'expected literature continuation indentation');
  assert.ok(defaultContinuation.textIndent > literatureContinuation.textIndent, 'expected literature continuation indentation to be reduced');
}
...
function testLiteratureCodeBlockModes(): void {
  ...
  const blocks = ir.elements.filter((element) => element.type === 'd2f_code_block');
...
function testAcademicTheoremCodeBlockMode(): void {
  const markdown = [
    '```theorem',
    'Theorem (Test): Stable motif index converges.',
    '```'
  ].join('\n');
  const inputPath = 'academic-theorem-mode.md';
  const ir = compileMarkdownToVmprint(markdown, inputPath, 'markdown', 'academic').ir;

  const block = ir.elements.find((element) => element.type === 'd2f_code_block');
...
function testTaskListMarkersRenderAsPrintMarkers(): void {
  ...
  const defaultIr = compileMarkdownToVmprint(markdown, inputPath, 'markdown', 'default').ir;
  const academicIr = compileMarkdownToVmprint(markdown, inputPath, 'markdown', 'academic').ir;

  const defaultRows = defaultIr.elements.filter((element) => String(element.type).startsWith('d2f_list_item_unordered_'));
  const academicRows = academicIr.elements.filter((element) => String(element.type).startsWith('d2f_list_item_unordered_'));
...
function testDefinitionListFallbackRendersDtDd(): void {
  const markdown = [
    'Signal lock',
    ': The line-break pattern remains deterministic across reruns.'
  ].join('\n');
  const inputPath = 'definition-fallback.md';
  const ir = compileMarkdownToVmprint(markdown, inputPath, 'markdown', 'default').ir;

  const hasTerm = ir.elements.some((element) => element.type === 'd2f_definition_term');
  const hasDesc = ir.elements.some((element) => element.type === 'd2f_definition_desc');
...
function testFlavorOrderedMarkerStyles(): void {
  ...
  const literatureIr = compileMarkdownToVmprint(markdown, inputPath, 'markdown', 'literature').ir;
  const ordered = literatureIr.elements.filter((element) => String(element.type).startsWith('d2f_list_item_ordered_'));
  assert.ok(ordered.length >= 3, 'expected ordered list output');

  const topLevel = ordered.find((element) => String(element.type).endsWith('_0'));
...
function testReferencePolicyByFlavor(): void {
  ...
  assert.ok(serialized.includes('"d2f_references_heading"'), 'expected notes/references heading');
...
function testLeadInParagraphKeepsWithFollowingDisplayBlock(): void {
  const markdown = [
    'When evening settled, one of them recited:',
    '',
    '```verse',
    'line one',
    'line two',
  ...
  const leadIn = ir.elements.find((element) =>
    element.type === 'd2f_paragraph' && JSON.stringify(element).includes('When evening settled, one of them recited:')
  );
...
function testMarkdownImageLocalFileCompiles(): void {
  ...
  const ir = compileMarkdownToVmprint(markdown, markdownPath, 'markdown').ir;
...
function testMarkdownImageRemoteUrlFailsClearly(): void {
  ...
  assert.throws(
    () => compileMarkdownToVmprint(markdown, inputPath, 'markdown'),
    (error: unknown) => {
...
function testOpenSourceFlavorFigureCaptionAndImageFrame(): void {
  ...
  const ir = compileMarkdownToVmprint(markdown, inputPath, 'markdown', 'opensource').ir;
  const imageElement = ir.elements.find((element) => element.type === 'image');
  assert.ok(imageElement, 'expected image element');
  ...
  const hasBlockquote = ir.elements.some((element) => element.type === 'd2f_blockquote');
  assert.equal(hasBlockquote, false, 'expected blockquote directly under image to be treated as figure caption');

  const captionElement = ir.elements.find((element) => element.type === 'd2f_paragraph');
...
function testOpenSourceFlavorTitleSubheading(): void {
  ...
  const subheading = ir.elements.find((element) => element.type === 'd2f_subheading');
...
function testOpenSourceFlavorLinksRenderAsFootnotes(): void {
  ...
  assert.ok(serialized.includes('"d2f_references_heading"'), 'expected footnotes heading element');
...
function testScreenplayFormatCompileAndSemanticMapping(): void {
  ...
  const result = compileMarkdownToVmprint(markdown, inputPath, 'screenplay');
  const ir = result.ir;

  assert.equal(result.format, 'screenplay');
  assert.ok(ir.styles.d2f_sp_title);
  assert.ok(ir.styles.d2f_sp_title_meta);
  assert.ok(ir.styles.d2f_sp_title_contact);
  assert.ok(ir.styles.d2f_sp_scene_heading);
  assert.ok(ir.styles.d2f_sp_action);
  assert.ok(ir.styles.d2f_sp_character);
  assert.ok(ir.styles.d2f_sp_parenthetical);
  assert.ok(ir.styles.d2f_sp_dialogue);
  assert.ok(ir.styles.d2f_sp_transition);
  assert.ok(ir.styles.d2f_sp_more);

  const emittedTypes = new Set(ir.elements.map((element) => element.type));
  assert.ok(emittedTypes.has('d2f_sp_title'));
  assert.ok(emittedTypes.has('d2f_sp_title_meta'));
  assert.ok(emittedTypes.has('d2f_sp_title_contact'));
  assert.ok(emittedTypes.has('d2f_sp_scene_heading'));
  assert.ok(emittedTypes.has('d2f_sp_action'));
  assert.ok(emittedTypes.has('d2f_sp_character'));
  assert.ok(emittedTypes.has('d2f_sp_parenthetical'));
  assert.ok(emittedTypes.has('d2f_sp_dialogue'));
  assert.ok(emittedTypes.has('d2f_sp_transition'));

  const dialogue = ir.elements.find((element) => element.type === 'd2f_sp_dialogue');
  ...
  const title = ir.elements.find((element) => element.type === 'd2f_sp_title');
  ...
  const firstScene = ir.elements.find((element) => element.type === 'd2f_sp_scene_heading');
...
function testScreenplayDialogueWrappingParagraphsAndHardBreaks(): void {
  ...
  const result = compileMarkdownToVmprint(markdown, inputPath, 'screenplay');
  const dialogue = result.ir.elements.find((element) => element.type === 'd2f_sp_dialogue');
  assert.ok(dialogue, 'expected screenplay dialogue element');

  const content = flattenElementText(dialogue);
  assert.ok(
    content.includes('This line is intentionally wrapped in source but should be a single wrapped paragraph in output.'),
...
function testScreenplayInlineStylingInActionAndTitle(): void {
  ...
  const title = result.ir.elements.find((element) => element.type === 'd2f_sp_title');
  ...
  const action = result.ir.elements.find((element) => element.type === 'd2f_sp_action');
...
function testScreenplayInlineStylingInDialogue(): void {
  const markdown = [
    ...
    '',
    '> @Maya',
    '> We need **deterministic** rollback *now*.'
  ].join('\n');
...
function testScreenplayDefaultIndustryGeometryProfile(): void {
  ...
  const titleStyle = (ir.styles.d2f_sp_title || {}) as Record<string, unknown>;
...
function testScreenplayContinuationCarriesParentheticalMetadata(): void {
  ...
  const ir = compileMarkdownToVmprint(markdown, inputPath, 'screenplay').ir;
  const dialogue = ir.elements.find((element) => element.type === 'd2f_sp_dialogue');
  assert.ok(dialogue, 'expected dialogue element');

  const continuation = dialogue?.properties?.paginationContinuation as Record<string, any> | undefined;
  assert.ok(Array.isArray(continuation?.markersBeforeContinuation), 'expected markersBeforeContinuation array');
  assert.equal(continuation?.markersBeforeContinuation?.length, 2, 'expected cue + parenthetical continuation markers');
  assert.equal(continuation?.markersBeforeContinuation?.[0]?.type, 'd2f_sp_character');
  assert.equal(continuation?.markersBeforeContinuation?.[1]?.type, 'd2f_sp_parenthetical');
...
function testScreenplayDualDialogueCuePairMapsToDualRoles(): void {
  ...
  const ir = compileMarkdownToVmprint(markdown, inputPath, 'screenplay').ir;
  const emittedTypes = new Set(ir.elements.map((element) => element.type));
  assert.equal(emittedTypes.has('d2f_sp_character_dual_left'), true, 'expected dual-left character role');
  assert.equal(emittedTypes.has('d2f_sp_dialogue_dual_left'), true, 'expected dual-left dialogue role');
  assert.equal(emittedTypes.has('d2f_sp_character_dual_right'), true, 'expected dual-right character role');
  assert.equal(emittedTypes.has('d2f_sp_dialogue_dual_right'), true, 'expected dual-right dialogue role');
  assert.equal(containsPropertyKey(ir.elements, 'screenplay'), false, 'expected no screenplay metadata in emitted VMPrint elements');
}

function testScreenplayProductionFlavorPrimitives(): void {
  const inputPath = path.resolve('src/formats/screenplay/flavors/sample_default.md');
  const markdown = fs.readFileSync(inputPath, 'utf-8');
  const ir = compileMarkdownToVmprint(markdown, inputPath, 'screenplay', 'production').ir;

  assert.equal(ir.layout.pageNumberFormat, '{n}.A', 'expected locked-pages revision suffix in page number format');
  const firstScene = ir.elements.find((element) => element.type === 'd2f_sp_scene_heading');
  assert.ok(firstScene, 'expected at least one scene heading');
  const firstSceneText = flattenElementText(firstScene);
  assert.ok(/^1 /.test(firstSceneText), 'expected prefixed scene number');
  assert.ok(/ 1$/.test(firstSceneText), 'expected suffixed scene number');
}
...
async function testScreenplayLayoutGoldenContinuationSignature(): Promise<void> {
  ...
  const page2More = page2.boxes.find((box: any) => box.type === 'd2f_sp_more');
  ...
  const contCue = page3.boxes.find((box: any) =>
    box.type === 'd2f_sp_character' && flattenBoxText(box).includes("CONT'D")
  );
  assert.ok(contCue, 'expected continuation cue on next page');

  const contParenthetical = page3.boxes.find((box: any) =>
    box.type === 'd2f_sp_parenthetical'
  );
  assert.ok(contParenthetical, 'expected continuation parenthetical on next page');

  const contDialogue = page3.boxes.find((box: any) =>
    box.type === 'd2f_sp_dialogue'
  );
...
function testCliIntegrationMarkdown(): void {
  const fixturePath = path.resolve('src/formats/markdown/flavors/sample_default.md');
...
async function run(): Promise<void> {
  ...
  testScreenplayProductionFlavorPrimitives();
