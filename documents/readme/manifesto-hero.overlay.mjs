...
function metricRule(ctx, y, xa, xb, label, opacity) {
  ...
  ctx.moveTo(xa, y - 3.5).lineTo(xa, y + 3.5).stroke(); // left tick
  ctx.moveTo(xb, y - 3.5).lineTo(xb, y + 3.5).stroke(); // right tick
  ctx.restore();

  if (label) {
    ctx.save();
    ctx.font("Helvetica", 5.2).fillColor(INK).opacity(opacity * 0.90);
...
function vDim(ctx, x, y1, y2, label) {
  ...
  ctx.save();
  ctx.strokeColor(INK).lineWidth(0.40).opacity(0.52);
  ctx.moveTo(x, y1).lineTo(x, y2).stroke();
  ctx.moveTo(x, y1).lineTo(x - tip * 0.55, y1 + tip).stroke();
  ctx.moveTo(x, y1).lineTo(x + tip * 0.55, y1 + tip).stroke();
  ctx.moveTo(x, y2).lineTo(x - tip * 0.55, y2 - tip).stroke();
  ctx.moveTo(x, y2).lineTo(x + tip * 0.55, y2 - tip).stroke();
  ctx.lineWidth(0.28).opacity(0.35);
  ctx.moveTo(x - 5, y1).lineTo(x + 5, y1).stroke();
  ctx.moveTo(x - 5, y2).lineTo(x + 5, y2).stroke();
  ctx.restore();
  ...
  ctx.rotate(-90);
  ctx.font("Helvetica", 5.0).fillColor(INK).opacity(0.50);
...
function rtlArrow(ctx, x, y, shaftLen) {
  var tip = 3;
  ctx.save();
  ctx.strokeColor(INK).lineWidth(0.38).opacity(0.50);
  ctx.moveTo(x, y).lineTo(x + shaftLen, y).stroke();
  ctx.moveTo(x, y).lineTo(x + tip, y - 2.2).stroke();
  ctx.moveTo(x, y).lineTo(x + tip, y + 2.2).stroke();
  ctx.restore();
}
...
export default {
  ...
  backdrop(page, ctx) {
    ...
    for (var y = 40; y < page.height - 38; y += step) {
      ctx.moveTo(36, y).lineTo(page.width - 36, y).stroke();
    }
    ctx.undash();
    ctx.restore();
  },
  ...
  overlay(page, ctx) {
    ...
    if (page.index === 0) {
      ...
      if (titleBox) {
        ...
        if (tm) {
          ...
          ctx.moveTo(lm - 10, tm.baseline).lineTo(rm + 10, tm.baseline).stroke();
          ctx.restore();
        }
      }
      ...
      if (dropCapBox) {
        ...
        if (dm) {
          ...
          metricRule(ctx, capBase, lm - RX, rm + RX, "BASELINE",   0.60);
          ...
          ctx.save();
          ctx.strokeColor(INK).lineWidth(0.30).dash(1.8, { space: 2.4 }).opacity(0.20);
      ...
      if (manifestoBox) {
        ...
        if (mm) {
          ...
          ctx.undash();
          ctx.restore();
          lbl(ctx, rm + LABEL_GAP, bodyAscY - 4, "ASCENDER", 4.8, 0.40);
        }
      }
      ...
      if (polyBox) {
        ...
        var pbY = pm
          ? pm.baseline - pm.ascent * 0.5
          : polyBox.y + safeN(polyBox.h, 14) * 0.5;
    ...
    if (page.index === 1) {
      ...
      if (cjkBox) {
        ...
        ctx.save();
        ctx.strokeColor(INK).lineWidth(0.24).dash(1.5, { space: 3 }).opacity(0.20);
        ctx.moveTo(lm - RX, cjkBottom).lineTo(rm + RX, cjkBottom).stroke();
        ctx.undash();
        ctx.restore();

        if (cjkBox.h > 4) {
          vDim(ctx, rm + 18, cjkBox.y, cjkBottom,
               safeN(cjkBox.h, 0).toFixed(1) + " pt");
        }
      }
      ...
      ctx.save();
      ctx.strokeColor(INK).lineWidth(0.28).opacity(0.20);
