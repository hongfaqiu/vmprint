...
function modeLabel(mode, align, wrap) {
  const m = mode === 'story-absolute' ? 'story-abs'
          : `float-${(align ?? 'L')[0].toUpperCase()}`;
  ...
  overlay(page, ctx) {
    ...
    if (page.index === 0) {
      ...
      const lt = (str, x, y, color, op = 0.85) => {
        ctx.save(); ctx.fillColor(color).opacity(op); ctx.font('Helvetica', 5.5);
        ctx.text(str, x, y, { lineBreak: false }); ctx.restore();
      };
      ...
      // Row 0 — image frame colors
      {
        ...
        [['#f59e0b', 0], ['#0d9488', 8], ['#8b5cf6', 16], ['#0ea5e9', 24], ['#ec4899', 32]].forEach(([c, dx]) => {
          ctx.save(); ctx.strokeColor(c).lineWidth(0.7).dash(3, { space: 2 }).opacity(0.8);
          ctx.rect(ICON_X + dx, iconY - 4, 6, 7).stroke(); ctx.undash(); ctx.restore();
        });
      ...
      // Row 1 — exclusion zone
      {
        ...
        ctx.save(); ctx.fillColor('#0d9488').opacity(0.14);
        ctx.rect(ICON_X, iconY - 4, 20, 8).fill(); ctx.restore();
        ctx.save(); ctx.strokeColor('#0d9488').lineWidth(0.7).dash(3, { space: 2 }).opacity(0.8);
        ctx.rect(ICON_X + 2, iconY - 2, 14, 4).stroke(); ctx.undash(); ctx.restore();
      ...
      // Row 2 — gap callout
      {
        ...
        ctx.save(); ctx.strokeColor('#0d9488').lineWidth(0.5).opacity(0.65);
        ctx.moveTo(ICON_X, iconY).lineTo(ICON_X + 12, iconY).stroke();
        ctx.moveTo(ICON_X, iconY - 2).lineTo(ICON_X, iconY + 2).stroke();
        ctx.moveTo(ICON_X + 12, iconY - 2).lineTo(ICON_X + 12, iconY + 2).stroke();
        ctx.restore();
      ...
      // Row 3 — per-line interval tints
      {
        ...
        ctx.save(); ctx.fillColor('#f59e0b').opacity(0.35);
        ctx.rect(ICON_X, iconY - 3, 12, 6).fill(); ctx.restore();
        ctx.save(); ctx.fillColor('#ec4899').opacity(0.25);
        ctx.rect(ICON_X + 12, iconY - 3, 8, 6).fill(); ctx.restore();
      ...
      // Row 4 — clear-point rule
      {
        ...
        ctx.save(); ctx.strokeColor('#0d9488').lineWidth(0.5).dash(3, { space: 3 }).opacity(0.6);
        ctx.moveTo(ICON_X, iconY).lineTo(ICON_X + 20, iconY).stroke(); ctx.undash(); ctx.restore();
      ...
      // Row 5 — mode pill
      {
        ...
        ctx.save(); ctx.fillColor('#0d9488').opacity(0.80);
        ctx.roundedRect(ICON_X, iconY - 4, 22, 8, 2).fill(); ctx.restore();
        ctx.save(); ctx.fillColor('#ffffff').opacity(1); ctx.font('Helvetica', 4.5);
        ctx.text('float-R', ICON_X + 2, iconY - 2, { lineBreak: false }); ctx.restore();
        lt('layout mode · wrap strategy', DESC_X, descY, '#374151');
      }

      // Optical underhang note
      ctx.save(); ctx.fillColor('#6366f1').opacity(0.70); ctx.font('Helvetica', 5.5);
      ctx.text('opticalUnderhang: on', lx + 4, ly + lh + 5, { lineBreak: false }); ctx.restore();
    }
    ...
    if (page.index > 0) {
      ...
      ctx.moveTo(LEFT_EDGE, gm + 4).lineTo(RIGHT_EDGE, gm + 4).stroke();
      ctx.fillColor('#94a3b8').opacity(0.7); ctx.font('Helvetica', 6);
    ...
    boxes.forEach(box => {
      ...
      if (box.type === 'image') {
        ...
        const gapLeft  = (wrap === 'around') && (align === 'right' || align === 'center' || (isStoryAbs && isRightHalf));
        const gapRight = (wrap === 'around') && (align === 'left'  || align === 'center' || (isStoryAbs && !isRightHalf));
        const gapTop   = wrap === 'top-bottom';
        const gapBot   = wrap === 'top-bottom';

        // ── Exclusion zone (SpatialMap registered rect) ──────────────────────
        if (wrap !== 'none') {
          let ezX = box.x, ezY = box.y, ezW = box.w, ezH = box.h;
          if (gapLeft)  { ezX -= gap; ezW += gap; }
          if (gapRight) { ezW += gap; }
          if (gapTop)   { ezY -= gap; ezH += gap; }
          if (gapBot)   { ezH += gap; }
          ctx.save();
        ...
        const drawGapArrow = (x1, y1, x2, y2, labelStr, labelSide) => {
          ...
          const isH = y1 === y2;
          if (isH) {
            ctx.moveTo(x1, y1 - 2).lineTo(x1, y1 + 2).stroke();
            ctx.moveTo(x2, y2 - 2).lineTo(x2, y2 + 2).stroke();
          } else {
            ctx.moveTo(x1 - 2, y1).lineTo(x1 + 2, y1).stroke();
            ctx.moveTo(x2 - 2, y2).lineTo(x2 + 2, y2).stroke();
          }
          ctx.restore();
          const lx2 = isH ? (labelSide === 'left' ? x1 - 14 : x2 + 2) : (x2 + 3);
          const ly2 = isH ? (y1 - 3) : ((y1 + y2) / 2 - 3);
          ctx.save(); ctx.fillColor(color).opacity(0.75); ctx.font('Helvetica', 5);
          ctx.text(labelStr, lx2, ly2, { lineBreak: false }); ctx.restore();
        };
        ...
        if (gap > 0) {
          ...
          if (gapRight) drawGapArrow(box.x + box.w, midY * 1.1, box.x + box.w + gap, midY * 1.1, gapStr, 'right');
        ...
        if (wrap === 'around') {
          ...
          ctx.save();
          ctx.strokeColor(color).lineWidth(0.5).dash(4, { space: 3 }).opacity(0.50);
          ctx.moveTo(LEFT_EDGE, clearY).lineTo(RIGHT_EDGE, clearY).stroke();
          ctx.undash();
          ctx.restore();
          ctx.save(); ctx.fillColor(color).opacity(0.60); ctx.font('Helvetica', 5);
          ctx.text('clear', LEFT_EDGE + 2, clearY - 6, { lineBreak: false }); ctx.restore();
        } else if (wrap === 'top-bottom') {
          [box.y - gap, box.y + box.h + gap].forEach(clearY => {
            ctx.save();
            ctx.strokeColor(color).lineWidth(0.5).dash(4, { space: 3 }).opacity(0.50);
            ctx.moveTo(LEFT_EDGE, clearY).lineTo(RIGHT_EDGE, clearY).stroke();
            ctx.undash(); ctx.restore();
          });
        }
        ...
        ctx.fillColor('#ffffff').opacity(1); ctx.font('Helvetica', 5.5);
        ctx.text(label, px + 4, pySafe + 2, { lineBreak: false });
        ctx.restore();

        // ── Dimensions label inside image ─────────────────────────────────────
        ctx.save(); ctx.fillColor('#ffffff').opacity(0.80); ctx.font('Helvetica', 5);
        ...
        if (isStoryAbs && storyX != null && storyY != null) {
          ctx.save(); ctx.fillColor('#ffffff').opacity(0.75); ctx.font('Helvetica', 5);
        ...
        if (wrap === 'none') {
          ...
          ctx.moveTo(box.x, box.y).lineTo(box.x + box.w, box.y + box.h).stroke();
          ctx.moveTo(box.x + box.w, box.y).lineTo(box.x, box.y + box.h).stroke();
          ctx.restore();
        }
      ...
      // ── Text / content boxes: per-line interval tints ───────────────────────
        ...
        metrics.lines.forEach((lineMeta, li) => {
          ...
          ctx.save(); ctx.fillColor('#f59e0b').opacity(0.08);
          ctx.rect(contentX, lineTop, lineTextWidth, lineH).fill(); ctx.restore();

          // Rose: the eaten space (exclusion zone encroachment)
          ctx.save(); ctx.fillColor('#ec4899').opacity(0.06);
          ctx.rect(contentX + lineTextWidth, lineTop, available - lineTextWidth, lineH).fill(); ctx.restore();
        });
      }
    });
  },
