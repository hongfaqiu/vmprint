...
function testFormatHandlerRoles(): void {
  for (const name of listFormats()) {
    ...
    assert.ok(roles.every((r) => typeof r === 'string'), `${name}: all roles should be strings`);
  }
}
...
function testFlavorContinuationIndentPolicy(): void {
  ...
  const defaultCont = defaultIr.elements.find((element) => String(element.type).startsWith('list-item-continuation-')) as any;
  const literatureCont = literatureIr.elements.find((element) => String(element.type).startsWith('list-item-continuation-')) as any;
...
function testTaskListMarkersRenderAsPrintMarkers(): void {
  ...
  const academicRows = academicIr.elements.filter((element) => String(element.type).startsWith('list-item-unordered-'));
...
function testLeadInParagraphKeepsWithFollowingDisplayBlock(): void {
  ...
  const leadIn = ir.elements.find((element) =>
    element.type === 'paragraph' && JSON.stringify(element).includes('When evening settled, one of them recited:')
  );
...
function testOpenSourceFlavorFigureCaptionAndImageFrame(): void {
  ...
  const ir = compileToVmprint(markdown, inputPath, {
    ...
    images: {
      frame: {
        mode: 'all',
        style: { borderWidth: 0.65, borderColor: '#8d99aa', borderRadius: 0, paddingTop: 11, paddingRight: 11, paddingBottom: 11, paddingLeft: 11, backgroundColor: '#fdfdfc' }
      }
    },
    captions: {
