...
function formatOrderedMarker(style: OrderedMarkerStyle, value: number, legalPath: number[]): string {
  switch (style) {
    case 'lower-alpha': return `${formatNumber(value, 'lower-alpha')}.`;
    case 'upper-alpha': return `${formatNumber(value, 'upper-alpha')}.`;
    case 'lower-roman': return `${toRoman(value, false)}.`;
    case 'upper-roman': return `${toRoman(value, true)}.`;
    case 'legal':       return `${[...legalPath, value].join('.')}.`;
    case 'decimal':
...
export class MarkdownFormat implements FormatHandler {
  ...
  flush(ctx: FormatContext): void {
    ...
    for (const entry of ctx.registeredLinks()) {
      ...
      const titleStr = includeTitle && entry.title && entry.title.trim().length > 0
        ? entry.title.trim()
        : undefined;
  ...
  roles(): string[] {
    return [
      ...
      'subheading', 'paragraph', 'blockquote', 'blockquote-attribution',
      'list-item-unordered', 'list-item-ordered', 'list-item-continuation',
      'code-block', 'thematic-break', 'inline-code', 'link', 'citation-marker',
      'references-heading', 'references-item', 'definition-term', 'definition-desc',
      'image'
    ];
  }
  ...
  private dispatchBlock(
    ...
    switch (node.kind) {
      ...
      case 'dd': {
        ...
        const isTitleSubheadingCandidate = subheadingEnabled
          && !inList
          && depth === 0
  ...
  private emitBlockImage(
    ...
    if (listContinuationRole) {
      const contStyle = ctx.getThemeStyle(`${listContinuationRole}-${contDepth}`) as Record<string, unknown> | undefined;
      baseStyle = {
        ...(contStyle || {}),
        marginTop: 0,
  ...
  private shouldFrameImage(imageNode: SemanticNode): boolean {
    ...
    const pattern = new RegExp(cfgStr(this.config, '\\b(frame|framed)\\b', 'images', 'frame', 'markerPattern'), 'i');
  ...
  private emitBlockquote(
    ...
    const contStyle = inList ? ctx.getThemeStyle(`${listContinuationRole}-${contDepth}`) as Record<string, unknown> | undefined : undefined;
    ...
    for (let idx = 0; idx < children.length; idx += 1) {
      ...
      if (asFigureCaption) {
        ...
        if (child.kind === 'p') {
          const captionStyle = captionBqStyle || cfg<Record<string, unknown>>(this.config, 'captions', 'style');
          const props: Record<string, unknown> = {
      ...
      const quoteChild = isAttribution && child.kind === 'p'
        ? { ...child, children: this.stripBlockquoteAttributionMarker(child.children || [], attributionPattern) }
        : child;
      ...
      if (child.kind === 'p') {
        const partStyle = isAttribution
          ...
          : { ...(bqStyle || {}), ...(contStyle || {}) };
        const role = isAttribution ? 'blockquote-attribution' : (inList ? `${listContinuationRole}-${contDepth}` : 'blockquote');
  ...
  private emitList(
    ...
    const unorderedMarkers = cfgArr<string>(this.config, ['\u2022', '\u25e6', '\u25aa'], 'list', 'unorderedMarkers');
    const orderedMarkers = cfgArr<string>(this.config, ['decimal', 'lower-alpha', 'lower-roman'], 'list', 'orderedMarkers');
    ...
    for (let index = 0; index < items.length; index += 1) {
      ...
      const nextOrderedPath = list.kind === 'ol'
        ? [...orderedPath, (list.start || 1) + index]
        : orderedPath;

      if (first.kind === 'p') {
        ctx.emit(styleType, [
          { kind: 'text', value: `${marker}${markerPad}` } as SemanticNode,
          ...(first.children || [])
        ], {
          style: baseStyle,
      ...
      for (let ci = 1; ci < itemChildren.length; ci += 1) {
        this.dispatchBlock(itemChildren[ci], ctx, depth + 1, `list-item-continuation`, nextOrderedPath, isTight);
      }
    }
  }
