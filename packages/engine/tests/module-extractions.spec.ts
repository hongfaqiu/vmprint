...
function testAppendSegmentMerge(): void {
    ...
    check(
        ...
        () => {
            const merged = appendSegmentToLine(line, next, 5, true, (left, right) => cache.areStylesEquivalent(left, right));
...
function testScriptSegmentationHelpers(): void {
    ...
    check(
        ...
        () => {
            ...
            assert.equal(runs.some((r) => r.isCJK), true);
            assert.equal(runs.some((r) => !r.isCJK), true);
        }
    );
    ...
    check(
        ...
        () => {
            const element: any = {
                ...
                content: '',
                children: [
                    { type: 'text', content: 'Heading title' }
                ]
            };

            const segments = getRichSegments(element, {}, {
    ...
    check(
        ...
        () => {
            ...
            registerFont({
                ...
                enabled: true,
                fallback: false
            }, runtimeA.fontRegistry, runtimeA.fontManager);
...
function testEmbeddedImageCacheCollisionSafety(): void {
    check(
        ...
        () => {
            ...
            assert.equal(parsedB.base64Data, dataB.replace(/^data:[^,]+,/, ''), 'second payload should preserve its own base64');
        }
    );
}
