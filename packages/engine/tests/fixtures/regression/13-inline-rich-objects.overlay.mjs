...
export default {
    overlay(page, ctx) {
        ...
        if (page.index === 0) {
            ...
            // Row 3-6: slot colors.
            [SLOT_STYLES[1], SLOT_STYLES[2], SLOT_STYLES[3], SLOT_STYLES[4]].forEach((style, i) => {
                const y = rowY(3 + i);
                ctx.save();
            ...
            box.lines.forEach((line, li) => {
                ...
                line.forEach((seg) => {
                    ...
                    if (previousObjectCenter) {
                        ...
                        ctx.moveTo(previousObjectCenter.x, previousObjectCenter.y).lineTo(cx, cy).stroke();
                        ctx.undash();
                        ctx.restore();
                    }
                    ...
                    textNoBreak(ctx, `${runTag}${style.label}`, segX + 1, lineTop - 6, 4.8, style.color, 0.78);
