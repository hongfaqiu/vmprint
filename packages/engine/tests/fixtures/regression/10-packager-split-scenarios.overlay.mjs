    ...
    overlay(page, ctx) {
        ...
        if (page.index === 0) {
            ...
            // Row 0 — box-type tints
            {
                const { descY } = row(0);
                ctx.save();
                ctx.fillColor('#c7d2fe').opacity(0.55);
                ctx.rect(ICON_X, descY, 7, 7).fill();
                ctx.fillColor('#fde68a').opacity(0.65);
                ctx.rect(ICON_X + 9, descY, 7, 7).fill();
                ctx.fillColor('#fda4af').opacity(0.7);
                ctx.rect(ICON_X + 18, descY, 7, 7).fill();
                ctx.restore();
            ...
            // Row 1 — table grid
            {
                ...
                ctx.moveTo(ICON_X + 7, iconY - 4)
                    .lineTo(ICON_X + 7, iconY + 4)
                    .stroke();
                ctx.moveTo(ICON_X + 14, iconY - 4)
                    .lineTo(ICON_X + 14, iconY + 4)
                    .stroke();
            ...
            // Row 2 — fragment badge + seam
            {
                ...
                ctx.moveTo(ICON_X + 16, iconY)
                    .lineTo(ICON_X + 24, iconY)
                    .stroke();
                ctx.undash();
                ctx.restore();
            ...
            // Row 5 — KWN bracket
            {
                ...
                ctx.moveTo(ICON_X + 10, iconY - 4)
                    .lineTo(ICON_X + 10, iconY + 4)
                    .stroke();
                ctx.moveTo(ICON_X + 10, iconY - 4)
                    .lineTo(ICON_X + 14, iconY - 4)
                    .stroke();
        ...
        boxes.forEach((box, bi) => {
            ...
            if (isFirstFrag && !isTableCell && !isSplitMarker) {
                ...
                if (scenLabel) {
                    ...
                    ctx.save();
                    ctx.fillColor('#ec4899').opacity(0.6);
            ...
            if (isTableCell && tableInfo?.topRowCells.includes(box)) {
                ...
                const pw = 18,
                    ph = 8;
                ctx.save();
                ctx.fillColor(color).opacity(0.7);
        ...
        kwnChains.forEach(({ start, end }) => {
            ...
            ctx.moveTo(bx, top)
                .lineTo(bx + 4, top)
                .stroke(); // top serif (toward content)
            ctx.moveTo(bx, bot)
                .lineTo(bx + 4, bot)
                .stroke(); // bottom serif
            ctx.restore();
            ...
            ctx.save();
            ctx.fillColor('#8b5cf6').opacity(0.8);
            ctx.font('Helvetica', 5.5);
        ...
        if (tableInfo) {
            ...
            cols.forEach((col, ci) => {
                if (ci > 0) {
                    ctx.save();
                    ctx.strokeColor('#10b981').lineWidth(0.4).dash(2, { space: 3 }).opacity(0.4);
            ...
            if (cols.length > 0) {
                ...
                ctx.save();
                ctx.strokeColor('#10b981').lineWidth(0.4).dash(2, { space: 3 }).opacity(0.4);
                ctx.moveTo(lastCol.x + lastCol.w, minY)
                    .lineTo(lastCol.x + lastCol.w, maxY)
                    .stroke();
                ctx.undash();
                ctx.restore();
            }
            ...
            if (calloutY >= 8) {
                cols.forEach((col) => {
                    ...
                    ctx.moveTo(col.x, calloutY + 4)
                        .lineTo(col.x + col.w, calloutY + 4)
                        .stroke();
                    ctx.moveTo(col.x, calloutY + 2)
                        .lineTo(col.x, calloutY + 6)
                        .stroke();
                    ctx.moveTo(col.x + col.w, calloutY + 2)
                        .lineTo(col.x + col.w, calloutY + 6)
                        .stroke();
                    ctx.restore();

                    ctx.save();
                    ctx.fillColor('#10b981').opacity(0.7);
                    ctx.font('Helvetica', 5);
