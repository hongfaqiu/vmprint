  ...
  overlay(page, ctx) {
    ...
    if (page.index === 0) {
      ...
      // Row 0 — Cap boundary rule
      {
        ...
        ctx.moveTo(ICON_X, iconY).lineTo(ICON_X + 18, iconY).stroke();
        ctx.undash();
        ctx.restore();
      ...
      // Row 2 — Gap callout
      {
        ...
        ctx.moveTo(ICON_X + 2, iconY).lineTo(ICON_X + 18, iconY).stroke();
        ctx.moveTo(ICON_X + 2, iconY - 2).lineTo(ICON_X + 2, iconY + 2).stroke();
        ctx.moveTo(ICON_X + 18, iconY - 2).lineTo(ICON_X + 18, iconY + 2).stroke();
        ctx.restore();
    ...
    boxes.forEach((box, bi) => {
      ...
      if (isDropcap) {
        ...
        ctx.moveTo(box.x, box.y + box.h).lineTo(RIGHT_EDGE, box.y + box.h).stroke();
        ctx.undash();
        // Left vertical bracket along the cap's left edge
        ctx.lineWidth(1).opacity(0.5);
        ctx.moveTo(box.x, box.y).lineTo(box.x, box.y + box.h).stroke();
        ctx.restore();
        ...
        ctx.moveTo(dimX, box.y).lineTo(dimX, box.y + box.h).stroke();
        ctx.moveTo(dimX - 3, box.y).lineTo(dimX + 3, box.y).stroke();
        ctx.moveTo(dimX - 3, box.y + box.h).lineTo(dimX + 3, box.y + box.h).stroke();
        ctx.restore();
        ctx.save();
        ...
        if (companion) {
          ...
          if (gap > 1) {
            ...
            ctx.moveTo(gapStart, arrowY - 2).lineTo(gapStart, arrowY + 2).stroke();
            ctx.moveTo(gapEnd,   arrowY - 2).lineTo(gapEnd,   arrowY + 2).stroke();
            ctx.restore();
            ctx.save();
          ...
          ctx.moveTo(companion.x, box.y).lineTo(companion.x, box.y + box.h).stroke();
          ctx.undash();
          ctx.restore();
          ...
          if (metrics?.lines) {
            metrics.lines.forEach((lineMeta) => {
              ...
              if (bl >= box.y - 1 && bl <= box.y + box.h + 1) {
                ...
                ctx.moveTo(box.x, bl).lineTo(box.x + box.w, bl).stroke();
                ctx.undash();
                ctx.restore();
              }
            });
