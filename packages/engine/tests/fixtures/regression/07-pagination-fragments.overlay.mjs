  ...
  overlay(page, ctx) {
    ...
    if (page.index === 0) {
      ...
      // Row 1 — Seam line
      {
        ...
        ctx.moveTo(ICON_X, iconY).lineTo(ICON_X + 16, iconY).stroke();
        ctx.undash();
        ctx.restore();
      ...
      // Row 6 — No-split bar
      {
        ...
        ctx.moveTo(ICON_X + 7, iconY - 4).lineTo(ICON_X + 7, iconY + 3).stroke();
        ctx.restore();
    ...
    boxes.forEach((box, bi) => {
      ...
      if (isCont) {
        ...
        ctx.save();
        ctx.strokeColor('#0d9488').lineWidth(0.9)
           .dash(3, { space: 2 }).opacity(0.8);
      ...
      if (noSplit) {
        ...
        ctx.save();
        ctx.strokeColor('#ec4899').lineWidth(2).opacity(0.55);
        ctx.moveTo(barX, box.y).lineTo(barX, box.y + box.h).stroke();
        ctx.restore();
      ...
      if (isBlockquote) {
        ...
        if (cm && cm.w > 0 && cm.h > 0) {
          ctx.save();
          ctx.strokeColor('#1a73e8').lineWidth(0.5)
             .dash(2, { space: 2 }).opacity(0.5);
